...
function App() {
  ...
  const [isEditAvatarOpen, setIsEditAvatarOpen] =
    useState(false);
  const [isAddPlaceOpen, setIsAddPlaceOpen] =
    useState(false);
  const [
    isEditProfileOpen,
    ...
    api
      ...
      .then((res) => {
        const newCards = cards.filter(
          (currentCard) =>
            currentCard._id !== cardToDelete._id
        );
        setCards(newCards);
      })
      .catch((err) => {
